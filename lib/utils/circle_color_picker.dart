...
class _CircleColorPickerState extends State<CircleColorPicker>
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        const SizedBox(height: 16),
                        _LightnessSlider(
                          key: Key(widget.lightness.toString()),
                          initialLightness:
                              widget.lightness ?? widget.initialLightness,
                          width: 140,
                          thumbSize: 26,
  ...
  void initState() {
    ...
    _lightnessController = AnimationController(
      vsync: this,
      value: widget.lightness ?? widget.initialLightness,
      lowerBound: 0,
      upperBound: 1,
    )..addListener(_onColorChanged);
    _colorController = AnimationController(
      vsync: this,
      value: widget.initialColor.value.toDouble(),
    )..addListener(_onInitialColorChanged);
  }
  ...
  void _onInitialColorChanged() {
    _lightnessController.value =
        HSLColor.fromColor(widget.initialColor).lightness;

    _hueController.value = HSLColor.fromColor(widget.initialColor).hue;
    setState(() {});
  }
}
